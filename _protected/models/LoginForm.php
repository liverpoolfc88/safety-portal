...
//			echo "</pre>";
//			die;
            $ADuser = User::findByAdUserName($this->username,'');
//			if (!$user || !Yii::$app->ad->auth()->attempt($this->username.'@and.ad.uzautomotors.com', $this->password)) {
////				$field = ($this->scenario === 'lwe') ? 'email' : 'username' ;
//				$this->addError($attribute, 'Incorrect login or password.');
//			}
            if (!Yii::$app->ad->auth()->attempt($this->username.'@'.$ADuser['account_suffix'], $this->password)) {
                $this->addError($attribute, Yii::t('app', 'AD: Incorrect password.'));
                return false;
            }
